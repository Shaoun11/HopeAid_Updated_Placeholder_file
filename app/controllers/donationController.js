import donationPlaceholder from "../models/donationModel.js";
// Controller for donation
export const donationDataCreate = async (req, res) => {
    try {
        // Your Code Here
        const reqBody=req.body;
        const data = await donationPlaceholder.create(reqBody);
...
export const donationDatas = async (req, res) => {
    try {
        const data = await donationPlaceholder.find();
        return res.status(200).json({ status: "success", data: data });
    } catch (error) {
        return res.status(400).json({ status: "fail", data: error });
    }
...
export const donationDataByID = async (req, res) => {
    try {
        const data = await donationPlaceholder.findById(req.params.id);
        if (!data) {
